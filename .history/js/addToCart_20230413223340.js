$(document).ready(function(){
    ...
    $('#nike-dunk-low').on('click',function(){
        console.log("clicked")
        if(localStorage.getItem('items')){
            items = JSON.parse(localStorage.getItem('items'))
            for (let j = 0; j < items.length; j++){
            ...
            items.push({name:'Nike dunk low',price: '59.99', img:'media/images/men6.jpg', quantity:'1'})
            localStorage.setItem('items',JSON.stringify(items))
           }
           else{
            items.push({name:'Nike dunk low',price: '59.99', img:'media/images/men6.jpg',quantity:'1'})
            localStorage.setItem('items',JSON.stringify([{name:'Nike dunk low',price: '59.99', img:'media/images/men6.jpg',quantity:'1'}]))
           }
        window.alert("Item Added")
    })
})